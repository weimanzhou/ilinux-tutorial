...
typedef struct process_s {
     ...
     * 因为 ILINUX 当前版本并不加以细分这几个段，它们都使用同一个
     * 段区域，所以一个就可以描述了。
     */
    memory_map_t map;
...
/*************************************************************************
    > bill_proc指向正在对其CPU使用计费的进程。当一个用户进程调用文件系统,
    > 而文件系统正在运行时,curr_proc(在global.h中)指向文件系统进程,
    > 但是bill_proc将指向发出该调用的用户进程。
    > 因为文件系统使用的CPU时间被作为调用者的系统时间来计费。
 ************************************************************************/
EXTERN process_t* bill_proc;

/*************************************************************************
    > 两个数组ready_head和ready_tail用来维护调度队列。
    > 例如,ready_head[TASK_Q]指向就绪任务队列中的第一个进程。
    > 就绪进程队列一共分为三个:
    >   ready_head[TASK_QUEUE]：    就绪系统任务队列
    >   ready_head[SERVER_QUEUE]：  就绪系统服务队列
    >   ready_head[USER_QUEUE]：    就绪用户进程队列
    >
    > 再举个例子，我们需要拿到用户进程队列的第3个进程,
    > 则应该这么拿：ready_head[USER_QUEUE]->next_ready->next_ready, 简单吧？
 ************************************************************************/
